...
- **HyperSniper Bot**: https://github.com/srobex/TGTON2 — Telegram бот для пользователей

## Требования

- Go 1.23+
- Docker 24+ и docker-compose (опционально)
- PostgreSQL 16 (используется в docker-compose, опционально)
- Redis 7+
...
Индексер может отправлять события о новых токенах в торговый бот через webhook.

### Настройка

1. Запустите бот с FastAPI на порту 8000 (см. документацию бота)
...
3. Перезапустите индексер

### Проверка интеграции

```bash
# Проверьте что бот готов принимать события
...
Индексер отправляет POST запрос с JSON:
...
Для развёртывания бота и индексера вместе, см.:
- `TGTON2/docs/РАЗВЁРТЫВАНИЕ_ПОЛНЫЙ_СТЕК.md`
...
- Подробный план развития: `PLAN.md`
